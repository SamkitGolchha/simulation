...
# Produces a 3-subplot figure (one per octahedron) with one line per simulation run.
...
# ---------------------------------------------------------------------------
# Subplot titles per octahedron body_id
# ---------------------------------------------------------------------------
_OCT_TITLES: Dict[int, str] = {
    0: "Octahedron 1 (bottom, grounded via sphere)",
    1: "Octahedron 2 (middle)",
    2: "Octahedron 3 (top, driven via sphere)",
}
...
def parse_run_csv(csv_path: str) -> RunData:
    # Parse one sim CSV and return per-body lists of (time, tilt_angle) pairs.
    # Accumulate (time, q) per body first, then compute angles in time order.
    ...
    with open(csv_path, newline="") as fh:
        ...
        for row in reader:
            bid = int(row["body_id"])
            if bid > 2:
                continue  # only octahedra have meaningful tilt
            t = float(row["time"])
            q0 = float(row["q0"])
            q1 = float(row["q1"])
            q2 = float(row["q2"])
...
def plot_tilt_angles(output_dir: str) -> None:
    # Load all runs, build the 3-subplot tilt-angle figure, and save to PNG.
