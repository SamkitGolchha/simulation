# Reads all sim_*.csv files from a directory and computes cube tilt angles.
...
#
# Faithful 1:1 port of 3x3x3_simulation/src/plot_tilts.py — the 3x3x3 grid and
# the (0,0) column body IDs are identical; only the unit-cell name changes
# (octahedron -> cube) in the subplot titles.
...
# Reference "up" = a cube's local +Z axis in its initial 45°-about-Y diamond
# pose. Tilt is measured against this (not world +Z), so a cube at rest in the
# diamond pose reads 0° — mirroring _cube_tilt_rad in src/simulation_cuboid.py.
_DIAMOND_TILT_RAD: float = float(np.radians(45.0))
_REF_UP: np.ndarray = np.array(
    [np.sin(_DIAMOND_TILT_RAD), 0.0, np.cos(_DIAMOND_TILT_RAD)]
)


def compute_tilt_angle(q0: float, q1: float, q2: float, q3: float) -> float:
    # Tilt (deg) = angle between the cube's current +Z (in world) and its initial
    # diamond +Z (_REF_UP). Measured relative to the 45° start so the rest pose
    # reads 0°, matching _cube_tilt_rad in simulation_cuboid.py.
    rot = Rotation.from_quat([q1, q2, q3, q0])  # scipy scalar-last: [qx,qy,qz,qw]
    z_world = rot.apply([0.0, 0.0, 1.0])
    cos_angle = float(np.clip(float(np.dot(z_world, _REF_UP)), -1.0, 1.0))
...
def plot_single_run(csv_path: str, out_path: str | None = None) -> str:
    ...
    for subplot_idx, body_id in enumerate(_COLUMN_00_BODY_IDS):
        ax = axes[subplot_idx]
        ax.set_title(_CUBE_TITLES[body_id], fontsize=11)
