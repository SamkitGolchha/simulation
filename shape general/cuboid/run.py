# Orchestrator script for the 3x3x3 cuboid tilt simulation pipeline.
...
def _variant_dirs() -> list[tuple[str, str]]:
    ...
    # the rigid baseline layout is preserved when the flag is flipped.
    from src.simulation_cuboid import JOINT_MODE, STIFFNESS_VARIANTS
    project_root = os.path.dirname(os.path.abspath(__file__))
    output_root = os.path.join(project_root, "output")
    if JOINT_MODE == "bushing":
        return [(label, os.path.join(output_root, label)) for label, *_ in STIFFNESS_VARIANTS]
...
def run_simulation() -> None:
    ...
    print("=" * 60)
    from src.simulation_cuboid import JOINT_MODE, run_all, run_sweep
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="3x3x3 Cuboid Tilt Simulation Pipeline"
    )
    ...
    print("\n" + "=" * 60)
    from src.simulation_cuboid import JOINT_MODE
    if JOINT_MODE == "bushing":
