"""Rest-state test for the 3x3x3 cuboid lattice (structure_cuboid).
...
Expected: max |v| < 1e-3 m/s. With no top load and no perturbation, the 464
bushings hold the cube lattice together as a stiff under-damped spring network
under gravity + ground contact, so it settles to rest.

Run with:

    DYLD_INSERT_LIBRARIES=$CONDA_PREFIX/lib/libomp.dylib \
        $CONDA_PREFIX/bin/python tests/rest_state_bushing.py
"""
...
def main() -> int:
    ...
    # Match the main loop's bushing dt = 1e-4.
    dt = 1e-4
    duration = 1.0
