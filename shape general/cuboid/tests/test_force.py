...
def analyze_run(csv_path: str, run_label: str) -> dict:
    ...
    # Tilt angles for all cubes (body_id 0-26)
    cube_tilts = {}
    for r in final_rows:
        bid = int(r["body_id"])
        if bid < 27:
    ...
    # Group by layer (9 cubes per layer)
    layer_tilts = {0: [], 1: [], 2: []}
    for bid, tilt in cube_tilts.items():
        iz = bid // 9
    ...
    # Time evolution of tilt for body 9 (iz=1, perturbed cube at (0,0,1))
    tilt_timeline = []
    for r in rows:
        if int(r["body_id"]) == 9:
            t = float(r["time"])
...
def main() -> None:
    ...
    for i in range(1, 6):
        ...
        print(f"  Tilt evolution (body 9 / perturbed cube):")
