...
def analyze_run(csv_path: str, run_label: str) -> dict:
    ...
    # Tilt angles for all octahedra (body_id 0-11)
    oct_tilts = {}
    for r in final_rows:
        bid = int(r["body_id"])
        if bid < 12:
    ...
    # Bottom spheres (12-15): check Z stability
    bot_z = {}
    initial_bot_z = {}
    for r in rows:
        bid = int(r["body_id"])
    ...
    # Top spheres (16-19): check descent
    top_z = {}
    initial_top_z = {}
    for r in rows:
        bid = int(r["body_id"])
    ...
    # Time evolution of tilt for body 4 (iz=1, perturbed octahedron)
    tilt_timeline = []
    for r in rows:
        if int(r["body_id"]) == 4:
            t = float(r["time"])
...
def main() -> None:
    ...
    for i in range(1, 6):
        ...
        for iz in range(3):
            ...
            max_tilt = max(tilts)
            min_tilt = min(tilts)
            print(f"  Layer iz={iz} (body {iz*4}-{iz*4+3}): "
        ...
        print(f"  Bottom spheres (body 12-15):")
        ...
        # Top spheres: descent
        print(f"  Top spheres (body 16-19):")
        for bid in sorted(result["top_dz"]):
            dz = result["top_dz"][bid]
            print(f"    body {bid}: dz={dz:.6f}")

        # Time evolution of body 4 tilt (sample every 5th entry ~= 0.25s)
        tl = result["tilt_timeline"]
        print(f"  Tilt evolution (body 4 / perturbed oct):")
