# Corner tower-view renderer for the 3x3x3 octahedral simulation.
...
# ---------------------------------------------------------------------------
# Grid + camera constants for the 3x3x3 lattice
# ---------------------------------------------------------------------------
_NX = 3
_NY = 3
_NZ = 3
...
_Z_TOP = (_NZ - 1) * _SPACING + _R              # top vertex of iz=NZ-1 octahedra
_STRUCT_HEIGHT = _Z_TOP - _Z_BOT                # ~4.24 for NZ=3

# Focal point: center of the structure
_FOCAL = (
    0.5 * (_NX - 1) * _SPACING,                 # center X across NX columns
    0.5 * (_NY - 1) * _SPACING,                 # center Y across NY columns
    0.5 * (_Z_BOT + _Z_TOP),                    # center Z
)

# Camera: corner tower view at 1.2x structure height
_CAM_Z = 1.2 * _STRUCT_HEIGHT
_CAM_DIST = 6.0 * max(_NX, _NY) / 2.0           # scale horizontal distance with grid width
_CAM_POS = (
