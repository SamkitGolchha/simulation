# Scaling Check 2: No Bottom Spheres — octahedra rest on ground directly via convex-hull collision.
# Standalone 2x2x3 lattice simulation with bottom spheres removed entirely.
# 16 bodies total: 12 octahedra (IDs 0-11) + 4 top spheres (IDs 12-15).
...
# ---------------------------------------------------------------------------
# Geometry helpers (inlined from Sanity_Test)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Build the full 2x2x3 system (no bottom spheres)
# ---------------------------------------------------------------------------

def build_system(
    perturbation_seed: int,
) -> tuple[chrono.ChSystemNSC, list[chrono.ChBody], list[str], int, list[chrono.ChBody]]:
    """Build the 2x2x3 lattice system with ball joints, collision ground, and top forces. No bottom spheres."""
    a = EDGE_LENGTH
    r = a / math.sqrt(2.0)
    ...
    # Ground body with collision floor at Z = -r (bottom octahedron vertex Z)
    ...
    # -------------------------------------------------------------------
    # Per-column boundary conditions: 4 top spheres only (no bottom spheres)
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # Body IDs: 0-11 octahedra, 12-15 top spheres
    all_bodies = oct_bodies + top_spheres
    all_names = oct_names + top_names
...
def run_all(n_runs: int = 5) -> list[str]:
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output_check2")
    paths: list[str] = []
